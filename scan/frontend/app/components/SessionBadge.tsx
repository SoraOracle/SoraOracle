      ...
      <div className="bg-s402-light-card dark:bg-gray-900 border border-gray-300 dark:border-gray-800 rounded-lg p-4 shadow-soft dark:shadow-none">
        ...
        <div className="space-y-2 text-sm">
          <div className="flex justify-between">
            ...
            <span className="font-medium">${session.spentAmount.toFixed(3)} USD1</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600 dark:text-gray-400">Remaining:</span>
            <span className="font-medium text-green-600 dark:text-green-400">${remaining.toFixed(3)} USD1</span>
          </div>
          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mt-2">
            <div 
              className="bg-s402-orange rounded-full h-2 transition-all duration-300"
              style={{ width: `${percentUsed}%` }}
            ></div>
          </div>
          <p className="text-xs text-gray-500 mt-1">
            💡 Close session to withdraw unused USD1 and BNB
          </p>
        </div>
      </div>
